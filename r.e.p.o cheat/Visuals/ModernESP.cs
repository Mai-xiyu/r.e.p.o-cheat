...
public class ModernESP : MonoBehaviour
{
	...
	private static bool _valueFieldSearched = false;
	...
	/// <summary>
	/// 获取缓存的价值字段反射信息，避免每帧重复查找
	/// </summary>
	private static FieldInfo GetValueField(ValuableObject item)
	{
		if (_cachedValueField != null) return _cachedValueField;
		if (_valueFieldSearched) return null;
		_valueFieldSearched = true;
		Type t = ((object)item).GetType();
		_cachedValueField = t.GetField("dollarValueCurrent", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
			?? t.GetField("dollarValue", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
	...
	private static void RenderItems()
	{
		...
		Camera main = GameHelper.GetActiveCamera();
	...
	private static void CreateItemLabel(ValuableObject item, Camera cam)
	{
		...
		{
			val.transform.LookAt(((Component)cam).transform.position);
	...
	private static void UpdateItemLabel(ValuableObject item, TextMeshPro label, Camera cam)
	{
		...
		// 反比例缩放：近处小、远处自动补偿，保持视觉大小稳定
		float fontSize = Mathf.Clamp(itemTextSize * num * 0.12f, 0.3f, itemTextSize);
		float num2 = 0f;
		FieldInfo fieldInfo = GetValueField(item);
		if (fieldInfo != null)
		{
		...
		if ((Object)(object)cam != (Object)null)
		{
			// 真正的 billboard 旋转：始终面向摄像机
			label.transform.LookAt(((Component)cam).transform.position);
	...
	private static void UpdateEnemyLabel(EnemyParent enemyParent, Enemy enemy, TextMeshPro label, Camera cam)
	{
		...
		// 反比例缩放保持视觉大小稳定
		float fontSize = Mathf.Clamp(enemyTextSize * num * 0.12f, 0.3f, enemyTextSize);
		((TMP_Text)label).fontSize = fontSize;
