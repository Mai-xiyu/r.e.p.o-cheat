...
public static class Teleport
{
	...
	private static bool CheckIfPlayerIsLocal(object playerObj)
	{
		...
		{
			// 先尝试 GameObject 比较
			if ((Object)(object)((Component)val2).gameObject == (Object)(object)localPlayer)
				return true;
			// Fallback: 通过 PhotonView ActorNumber 比较（修复 PlayerAvatar vs PlayerController GO 不匹配）
			try
			{
				FieldInfo field = val2.GetType().GetField("photonView", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				if (field != null)
				{
					PhotonView pv = field.GetValue(val2) as PhotonView;
					if (pv != null && pv.OwnerActorNr == PhotonNetwork.LocalPlayer.ActorNumber)
						return true;
				}
			}
			catch { }
