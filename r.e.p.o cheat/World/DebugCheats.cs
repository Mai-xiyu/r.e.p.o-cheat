...
internal static class DebugCheats
{
	...
	public static void DrawESP()
	{
		...
		// Ensure ESP box texture is valid (may be destroyed on scene transitions)
		if ((Object)(object)texture2 == (Object)null)
		{
			texture2 = new Texture2D(2, 2, (TextureFormat)5, false);
			texture2.SetPixels(new Color[4] { Color.red, Color.red, Color.red, Color.red });
			texture2.Apply();
		}
		InitializeStyles();
		if (!drawEspBool && !drawItemEspBool && !drawExtractionPointEspBool && !drawPlayerEspBool && !draw2DPlayerEspBool && !draw3DPlayerEspBool && !draw3DItemEspBool && !drawChamsBool && !SkeletonESP.enabled)
		{
			return;
		}
		if ((Object)(object)localPlayer == (Object)null)
		...
		if ((Object)(object)cachedCamera == (Object)null || (Object)(object)cachedCamera != (Object)(object)GameHelper.GetActiveCamera())
		{
			cachedCamera = GameHelper.GetActiveCamera();
		...
		{
			Camera main = cachedCamera ?? GameHelper.GetActiveCamera();
		...
		else
		{
			...
			if (enemyCachedOriginalCamera)
			{
				Camera main2 = cachedCamera ?? GameHelper.GetActiveCamera();
		...
		if (drawItemEspBool && drawItemChamsBool)
		{
			...
			Camera main3 = cachedCamera ?? GameHelper.GetActiveCamera();
		...
		if (drawItemEspBool)
		{
			...
			foreach (object valuableObject2 in valuableObjects)
			{
				...
				// 屏幕外的物品钦定到屏幕边缘
				float clampedX = Mathf.Clamp(val10.x, 20f, (float)Screen.width - 20f);
				float clampedY = Mathf.Clamp(val10.y, 20f, (float)Screen.height - 20f);
				float num18 = clampedX * scaleX;
				float num19 = (float)Screen.height - clampedY * scaleY;
		...
		if (drawExtractionPointEspBool)
		{
			...
			foreach (ExtractionPointData extractionPoint2 in extractionPointList)
			{
				...
				if (val11.z > 0f)
				{
					float num28 = Mathf.Clamp(val11.x, 20f, (float)Screen.width - 20f) * scaleX;
					float num29 = (float)Screen.height - Mathf.Clamp(val11.y, 20f, (float)Screen.height - 20f) * scaleY;
		...
		// 骨骼 ESP (2D 线条叠加)
		if (SkeletonESP.enabled && (Object)(object)cachedCamera != (Object)null)
		{
			SkeletonESP.RenderSkeletonDots(cachedCamera);
		}
	}
}
