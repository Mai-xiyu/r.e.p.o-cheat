...
public static class NoclipController
{
	...
	public static void ResetState()
	{
		...
		crouchAction = null;
		sprintAction = null;
		_tumbleFieldsSearched = false;
		_cachedTumbleType = null;
		_cachedTumbleActiveField = null;
		_cachedTumbleTimerField = null;
	}
	...
	// 缓存翻滚物理相关反射，避免每帧重复查找
	private static Type _cachedTumbleType;
	private static FieldInfo _cachedTumbleActiveField;
	private static FieldInfo _cachedTumbleTimerField;
	private static bool _tumbleFieldsSearched = false;
	...
	private static void CancelTumblePhysics()
	{
		try
		{
			...
			// 缓存反射信息，只查找一次
			if (!_tumbleFieldsSearched)
			{
				_tumbleFieldsSearched = true;
			...
			if (_cachedTumbleType == null) return;

			Component tumble = ((Component)(MonoBehaviour)playerControllerInstance).GetComponentInChildren(_cachedTumbleType);
			if (tumble == null) return;

			// 获取 tumbleActive 字段并设为 false
			if (_cachedTumbleActiveField != null)
			{
				bool isTumbling = (bool)_cachedTumbleActiveField.GetValue(tumble);
				if (isTumbling)
				{
					_cachedTumbleActiveField.SetValue(tumble, false);
				}
			}

			// 同时尝试设置 tumbleTimer 为 0
			if (_cachedTumbleTimerField != null)
			{
				_cachedTumbleTimerField.SetValue(tumble, 0f);
			}
		}
		catch { }
	}
