...
- **自身**：无敌、无限生命/体力、穿墙飞行、速度/力量/跳跃/重力/抓取距离自定义、无后坐力/无冷却、隔墙抓取、自定义 FOV、去雾、自动闪避、自我复活、治疗/复活队友、创造模式、一键满级（全部 13 种升级）、游戏速度控制、游戏调试旗标（无限能量/免疫过载/禁止翻滚）。
...
- **传送**：准星传送、撤离点传送、随机传送、命名路径点，经游戏自身的 `PlayerAvatar.Spawn` API 同步。
- **世界**：撤离目标归零、一键完成回合、聊天指令（`!help`）、商店工具（经游戏自身的 `SetRunStatSet` 修改货币）、经 `RunManager.ChangeLevel` 切换场景。
